...
def test_run_secure_validation(mock_run):
    ...
    banner, context, report, env_info, map_html = run_secure_validation("GPS(1.0, 2.0)", "sess1")
...
def test_run_secure_validation_empty():
    banner, context, report, env_info, map_html = run_secure_validation("   ", "sess1")
...
def test_run_adk_agent_not_initialized():
    ...
    svg, result, result2, result3 = execute_evaluation_suite()
    assert isinstance(result, dict) or isinstance(result, str)
    assert "suite" in result or "suite" in str(result)
