...
This application combines an interactive Frontend GUI Control Panel (built in Gradio) with an advanced, decoupled backend ADK 2.0 multi-agent pipeline. The system leverages state-of-the-art LLMs (Gemini 2.5 Pro and Gemini 2.5 Flash) orchestrating diverse deterministic tools.

The architecture is divided into the following core subsystems:

### Procedural Synthetic SDG Engine
To facilitate end-to-end testing without exposing confidential operator information or relying on scarce real-world anomalies, the platform incorporates an on-demand Synthetic Data Generation (SDG) skill. Powered by Gemini 2.5 Flash, this simulator dynamically builds messy, highly realistic testing logs. Gemini 2.5 Flash was strategically selected for this stage due to its high-throughput, low-latency generation capabilities, which are perfectly suited for rapidly spinning up massive volumes of diverse training data. It strategically injects placeholder PII (Names, Plates, GPS Coordinates) across varied weather and traffic scenarios, ensuring our downstream filters and evaluators are continuously tested against diverse edge cases.
...
The purified, redacted text passes into the orchestration layer where specialized Legal Auditor and Technical Analyst personas resolve rules intersections. This stage is powered by Gemini 2.5 Pro, which was chosen specifically for its deep reasoning compliance, superior handling of long complex contexts (such as multifaceted corporate rulebooks), and zero-shot reasoning capabilities needed for accurate legal/technical audits. Using dynamic Knowledge Retrieval (RAG) capabilities, the agent dynamically fetches domain glossaries, fleet hardware histories, and complex safety guardrails from local assets. Additionally, the orchestrator has tools to invoke third-party APIs (Google Maps Geocoding, Roads APIs for Speed Limits, and Open-Meteo for weather) to enrich the context. It correlates these disparate inputs to generate formally structured Kaggle-compliant compliance reports classifying incidents into CRITICAL or HIGH severity.
