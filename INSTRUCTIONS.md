...
3. **Result:** The system will use Gemini 1.5 Flash to synthesize a highly complex scenario involving heavy traffic, sensor obstructions, and injected Personal Identifiable Information (PII) like Driver Names, Plates, and GPS Coordinates.
...
3. **Result:**
   ...
   - **Stage 2 (Compliance Analysis):** The scrubbed text is sent securely to the Compliance Agent (Gemini 1.5 Pro). The agent will query the RAG assets, fetch Geolocation/Weather data, validate any telemetry gaps, and return a structured Kaggle-ready Compliance JSONL format detailing the incident severity. An interactive map frame will also render the incident location.
